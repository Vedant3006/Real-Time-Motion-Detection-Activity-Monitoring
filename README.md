...
Open a terminal or PowerShell and clone the GitHub repository:

```bash
git clone https://github.com/Vedant3006/Real-Time-Motion-Detection-Activity-Monitoring.git
```
...
The project was developed and tested using Python 3.12.6.
...
The project uses OpenCV and NumPy. All required packages are listed in `requirements.txt`.
...
The project uses the default webcam with camera ID `0`.

The camera ID is defined in `config.py`:

```python
CAMERA_ID = 0
```

If a different camera needs to be used, the `CAMERA_ID` value can be changed in `config.py`.

The same file also contains the motion detection settings, including the motion threshold and minimum contour area.

### Step 5: Run the Project

After completing the setup, run the following command from the project folder:
